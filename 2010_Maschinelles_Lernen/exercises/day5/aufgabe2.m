...
posSamplesMat = encodeMat(SeqPosMat);
negSamplesMat = encodeMat(SeqNegMat);

% Anzahl von positiven und negativen Beispielen
posDim = columns(posSamplesMat);
negDim = columns(negSamplesMat);
...
for kPar = 1:2:25
	...
	for nRuns = 1:10
		...
		codedPosMat2 = posSamplesMat(:, randperm(posDim));
		codedNegMat2 = negSamplesMat(:, randperm(negDim));
...
xlabel("k-Wert");
ylabel("Fehler [%]");
title("kNN-Klassifikator auf TIS-Datenmenge");
